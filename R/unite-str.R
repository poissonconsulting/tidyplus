...
unite_str <- function(data, col, ..., sep = ". ", remove = TRUE) {
  ...
  data <- dplyr::mutate(
    data,
    dplyr::across(tidyselect::all_of(from_vars), as.character)
  )
  data <- dplyr::mutate(
    data,
    dplyr::across(tidyselect::all_of(from_vars), na_if_blank)
  )
  data <- tidyr::unite(
    data,
    col = !!col,
  ...
  data <- dplyr::mutate(
    data,
    dplyr::across(tidyselect::all_of(rlang::as_string(col)), na_if_blank)
  )
  data
}
